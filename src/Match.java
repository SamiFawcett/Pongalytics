...
public class Match {
	...
	public Match(String match_id) {
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader(Client.MATCH_BASE_ALL_LOC));
			String[] tokens = null;
			if(br.ready()) {
				String line;
			...
			if(!Match.players_confirmed) {
				this.winner = Client.find(tokens[0], false);
				this.loser = Client.find(tokens[1], false);
			}
			Match.players_confirmed = true;
	...
	public static void start() throws Exception {
		...
		Player player_one = Client.find(k.read(), true);
		System.out.println("Player Two: ");
		Player player_two = Client.find(k.read(), true);
		...
		if(player_one_score > player_two_score) {
			...
			Client.write_match(player_one.get_handle(),player_one_score, player_two.get_handle(), player_two_score, m.get_id());
		} else {
			m.set_score(player_two_score, player_one_score);
			Client.write_match(player_two.get_handle(), player_two_score, player_one.get_handle(), player_one_score, m.get_id());
		}
